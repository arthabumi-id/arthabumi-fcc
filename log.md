...
- **(b) `scheduleRetry(op)`** — kegagalan transport kini **menyembuhkan diri**: kirim ulang otomatis pada **15 dtk lalu 30 dtk**, maksimum **2 percobaan**, sesudah itu diserahkan ke user. **HANYA aman karena penjaga idempoten v39 sudah live** — kirim ulang tidak bisa menulis dua kali. Ditembakkan **hanya dari cabang `catch`** (kegagalan transport); respons `{error}` dari server (mis. data ditolak) **TIDAK** di-retry karena mengulang tak akan menolong. Sebelum mengirim ulang: cek op masih ada di antrean DAN statusnya masih `failed` (kalau user sudah menekan Kirim ulang / sudah sukses / sudah dibuang → batal), lalu `warmBackend()` dulu.
- **Verifikasi:** `vm.Script` parse penuh OK; harness 9 test PASS — pemanasan menembak sekali lalu ter-throttle, tidak menembak sebelum login, jadwal 15/30 dtk, percobaan ke-3 ditolak, dan tiga pengaman anti-dobel-kirim (op hilang dari antrean → batal; op sedang `sending` → batal; op masih `failed` → jalan).
- APP_VERSION→**v39.1**, sw.js → **v82**, CHANGELOG +1. **Deploy: push index.html + sw.js saja — TIDAK perlu redeploy Code.gs.**

---

## SESSION — 2026-08-13 (v39 Pengaman Anti-Dobel / Idempotensi) — ⚠️ WAJIB REDEPLOY Code.gs, sw.js v81
- **✅ SUDAH LIVE:** backend **Version 39 (13 Agu 21:24)**, Deployment ID tetap `AKfycbyxgx44G45Vqkp8...`. **Uji terima di sistem hidup:** `addKat` dikirim **3× dengan OP_ID sama** → **tepat 1 baris** (`ID1786631327084`); kiriman ke-2 dibalas `{ok:true,id:…,dup:true}`; kiriman ke-3 kena 404 tapi **tetap tidak menambah baris**. Jejak uji dihapus (kategori 144→143).
- **⚠️ NYARIS CELAKA saat redeploy:** dropdown Version sempat meleset ke **Version 37** — kalau ter-Deploy, backend mundur ke kode 8 Juli (v38 & v39 hilang). Ketahuan karena screenshot sebelum klik. **ATURAN: selalu screenshot dropdown Version sebelum menekan Deploy.**
- **Pemicu — INSIDEN DATA DOBEL:** termin piutang "Pelunasan" project *Perbaikan Saklar dan Stop Kontak Lantai* (Bu Fifi, Rp 400.000, tempo 2026-08-11) tercatat **6×**, semua ber-ID sama `IDMSRIN9L6` (12:50:20 s/d 12:57:37 UTC). Piutang menggelembung Rp 2.000.000. PRD: `PRD-idempoten-v39.md` (sign-off Eddy: OQ1 60 hari, OQ2 retry otomatis dipertahankan, OQ3 tanpa tombol cek manual, OQ4 deployment baru ditunda).
- **AKAR (terukur, jangan didiagnosa ulang dari nol):** lapisan **pengantar respons Google** yang rusak — BUKAN kode kita, BUKAN v38. Probe POST berselang-seling: sukses 1,5–4 dtk; gagal **404 setelah 11–21 dtk**. Log Executions Apps Script menunjukkan **SEMUA `Completed`, 0,6–2,3 dtk** — termasuk yang di client jadi 404. Tes terkendali: probe 20:05:58/20:06:04/20:06:25 → eksekusi 8:05:57/8:06:03/8:06:24 semuanya Completed padahal dua di antaranya 404. **Artinya script jalan & data TERTULIS, hanya responsnya tak sampai.**
- **Rantai yang mengubah gangguan jadi kerusakan data:** (1) `sendOp` menandai `failed` tanpa verifikasi apa pun; (2) **`syncAll()` memanggil `retryPending()` di baris pertama** → tiap Sync/pull-refresh mengirim ulang otomatis (inilah sebab 6×, bukan Eddy menekan retry 6 kali); (3) bar antrean HANYA punya "Kirim ulang", tak ada tombol buang → user terjebak; (4) backend tanpa guard idempoten.
- **⚠️ TEMUAN YANG MENGUBAH DESAIN:** rencana awal "backend tolak ID yang sudah ada" **TIDAK CUKUP**. Audit Code.gs: aksi paling berbahaya justru **tidak menerima `data.ID`** — `addTxn`, `addTransfer`, `addCicilan`, `addReserve`, `addForexConvert`, `addTxnBatch` semuanya generate ID di server → tiap kiriman ulang jadi baris baru yang sah & mustahil di-dedup. Yang menerima `data.ID` cuma `addPiutang`/`addKasbon`/`addJadwal`/`addAddendum`. Ditambah banyak aksi menulis **beberapa baris sekaligus** (`payPiutang` = update piutang + 1 TXN; `addTransfer` = 2 TXN + TRANSFER_LOG) → dedup per-baris tak bisa melindunginya.
- **SOLUSI: penjaga di LEVEL OPERASI.** Sheet baru **`OP_LOG`** `['OP_ID','ACTION','RESULT','CREATED_AT']`. Di `doPost`, sebelum `switch`: ambil `LockService.getScriptLock()` (tryLock 20 dtk, gagal dapat lock → tetap lanjut best-effort), cari `OP_ID` di OP_LOG; **ketemu → balas `RESULT` asli + `dup:true` TANPA mengeksekusi apa pun**; tidak ketemu → eksekusi lalu catat. Lock dilepas di `finally`. Satu penjaga menutup SEMUA aksi termasuk yang majemuk, tanpa menyentuh logika satu pun fungsi (risiko regresi minimal).
- **Helper:** `_opSheet` / `_opLogFind` (baca **kolom OP_ID saja**, RESULT baru diambil kalau ketemu, telusur dari bawah = terbaru) / `_opLogReplay` / `_opLogWrite` (RESULT >5000 char dipangkas) / `_pruneOpLog` (OPLOG_KEEP_DAYS=60; baris kronologis → yang kedaluwarsa pasti blok atas, dibuang sekali `deleteRows`, dipicu hanya saat lastRow>1000).
- **Client:** `sendOp` mengirim `OP_ID: op.id` — **`op.id` sudah ada sejak dulu (`uid()`) dan TETAP SAMA saat dikirim ulang**, jadi idempotensinya gratis. `apiPost(body, opId)` +param opsional; tanpa opId tetap mengirim penanda sekali-pakai (tidak men-dedup, tapi semua operasi tulis jadi tercatat di OP_LOG = jejak audit). **Hanya ada 2 pemanggil `fetch(API_URL)`** (`apiPost` & `sendOp`) — dua-duanya sudah kirim OP_ID, jadi cakupannya menyeluruh. Tombol **Buang** (`discardPending`) di bar antrean; label bar "gagal tersimpan" → **"belum terkonfirmasi"**; `op.err` 'koneksi' → **'respons tidak kembali'** (kata "koneksi" menyesatkan — datanya justru sering sudah masuk).
- **Verifikasi:** `node --check` Code.gs OK; `vm.Script` parse penuh index.html OK; **harness backend 12 test PASS** (simulasi persis 6 kiriman kembar → 1 eksekusi; respons asli diputar ulang utuh; operasi majemuk 3× kirim tetap 1 set; prune 60 hari & idempoten; RESULT rusak tidak melempar); **harness client 6 test PASS** (hanya op `failed` yang dibuang, op `sending` selamat, Batal tidak menghapus apa pun).
- **⚠️ BATAS YANG DISADARI (belum ditutup):** kalau **user sendiri** mengulang aksi lewat jalur `apiPost` (mis. menekan Bayar dua kali karena yang pertama tampak gagal), tiap penekanan = OP_ID baru → **masih bisa dobel**. Yang tertutup penuh adalah jalur antrean `bgPost` (retry otomatis — sumber insiden ini). Menutup celah user-redo butuh sidik-jari isi transaksi, dan itu berisiko memblokir transaksi kembar yang SAH (mis. dua parkir Rp 20.000 di hari yang sama) — sengaja tidak diambil tanpa keputusan Eddy.
- APP_VERSION→**v39**, sw.js → **v81**, CHANGELOG +1. **Deploy: push index.html+sw.js (Eddy via GitHub Desktop) + paste Code.gs + REDEPLOY New Version.**

---

## SESSION — 2026-08-12 (v38 Rekap Isi Tagihan CC + riwayat) — ⚠️ WAJIB REDEPLOY Code.gs, sw.js v80
- **Permintaan Eddy:** "sudah input tagihan CC yang aktual, bisa nggak bikin rekapan biar gampang cek ada tagihan apa saja di dalam lembaran tagihan." PRD: `PRD-rekap-tagihan-cc-v38.md` (sign-off "Proceed", OQ1=a jujur-kosong, **OQ2=simpan 3 bulan**, OQ3=polos, OQ4=teks tanpa ID).
- **AKAR MASALAH (3 lapis, semuanya by design lama):** (1) baris yang dicentang saat `lockCCBillUI` langsung masuk `PAID_MARK`, sedangkan `renderCCBill` memfilter `!isPaid(t.ID)` → **justru sesudah dikunci isi tagihannya hilang dari layar**; (2) sheet `CC_TAGIHAN` tak pernah menyimpan daftar barisnya (cuma NOMINAL/tempo/periode); (3) saat dibayar barisnya **di-`deleteRow`** → riwayat statement nol.
- **Code.gs (REDEPLOY):** HEADERS `CC_TAGIHAN` +`RINCIAN`,`TGL_BAYAR`; const `CCBILL_KEEP_MONTHS=3`; helper `_ccbSheet` (buat sheet + ensureCol idempoten — **wajib sebelum appendRow**, kalau tidak appendRow menulis melewati header dan getSheet menghasilkan key kosong), `_ccbLite`, `_ccbClampRincian`, `_pruneCCBills`; `lockCCBill` kini **hanya menghapus baris STATUS≠'Lunas'** + simpan RINCIAN + prune; action baru **`settleCCBill`** (STATUS→Lunas + TGL_BAYAR, menggantikan deleteRow) & GET **`getCCBillDetail`**.
- **⚠️ KEPUTUSAN PERFORMA (jangan dibalik):** `getBundle` memakai `_ccbLite` yang **membuang kolom RINCIAN** dan menggantinya dengan boolean `HAS_RINCIAN`. Kalau snapshot ikut bundle, ±6 kartu × 12 statement/tahun × beberapa KB membuat payload sync membengkak permanen. Rincian diambil satu-satu lewat `getCCBillDetail` hanya saat popup dibuka.
- **index.html:** `_ccbRows` + `_ccbShowPaid`; checkbox membawa **`data-i` (indeks), bukan label** — `esc()` tidak meng-escape tanda kutip sehingga NOTES di atribut HTML gampang merusak markup; toggle "Tampilkan baris yang sudah masuk tagihan lain" (baris paid tampil abu, **disabled**, tidak ikut subtotal); `lockCCBillUI` merakit snapshot `{v,cc,bill,stmt,sum,dari,sampai,tempo,lines[{d,l,n,t}]}`; mode TAMBAHKAN menggabung rincian lama via `ccBillLines` dan **DIBATALKAN kalau fetch gagal** (snapshot separuh dgn nominal gabungan lebih menyesatkan daripada tak ada). Popup `openCCBillRecap`/`ccbRecapHTML` (A4 polos, judul kartu + bulan statement, grup Belanja/Angsuran/Kredit + subtotal, selisih vs statement, tombol Cetak & Tutup, `.noprint`); drawer `openCCBillHistory`; `ccbLockedInfoHTML` di atas drawer Rincian; tombol **Isi Tagihan** + **Riwayat** di kartu CC Master; link "lihat isi tagihan" di billStrip dashboard.
- **`_ccbRp` sengaja TIDAK memakai `fmt()`** — `fmt` menghormati "mode mata" (nominal jadi ••••); dokumen cetak tidak boleh ikut tersamar. Diuji di harness.
- **Fallback R2 (preseden lupa redeploy 8 Jul):** `settleCCBillLocal` → kalau `settleCCBill` ditolak server (backend belum redeploy) otomatis mundur ke `deleteRow` lama + toast peringatan; kalau apiPost `null` (offline) settle-nya masuk **antrian bgPost**, bukan langsung dihapus.
- **🐞 Bug yang tertangkap saat verifikasi:** panggilan pertama di `doPayCicilanExec` memakai variabel `tgl` yang **tidak ada di scope itu** (adanya di `saveCicilan`) → ReferenceError yang akan mematikan alur bayar angsuran. vm.Script tidak menangkapnya (bukan syntax error) — ketahuan dari pembacaan scope manual. Diganti `today()`. Di `saveDrawer` mode paycc, `tgl` memang ada (baris ~5290).
- **Verifikasi:** parse penuh `vm.Script` OK; `node --check` Code.gs OK; **harness klien 19 test PASS** (label bulan, escape `"`/`&`/`<>`, kredit dikurung, selisih, catatan pra-v38, status Lunas, mode-mata tidak bocor); **harness backend 12 test PASS** dengan mock Sheets (arsip Lunas selamat saat lock baru, prune 3 bln hanya kena Lunas & tidak pernah kena Aktif, settle by ID & by CC, clamp 45k tetap JSON valid). Grep: sisa `deleteRow CC_TAGIHAN` tinggal 2 — `clearCCBill` (Abaikan, memang harus hapus) & fallback R2.
- **Batas yang diterima Eddy:** tagihan yang dikunci SEBELUM v38 (mis. BMW) tidak punya snapshot → popup jujur bilang "rincian tidak tersedia", tidak direkonstruksi diam-diam (OQ1=a). Kunci ulang mode GANTI kalau mau snapshot asli. Tombol **Abaikan** tetap menghapus baris (memang hapus pengingat), jadi rekapnya ikut hilang.
- **Editing:** semua perubahan dilakukan pada **salinan lokal di sandbox** (hasil `device_stage_files`, 527.642 byte = identik ukuran di device, ekor file utuh), pakai Edit tool, lalu dikirim balik utuh via `device_commit_files`. TIDAK ada python/bash read-modify-write di mount E:\ (aturan pasca-insiden 13 Jul).
- **Deploy:** push `index.html` + `sw.js` via GitHub Desktop **DAN** paste `Code.gs` → **redeploy New Version**. APP_VERSION→**v38**, sw.js → **v80**.
...
- **v37.1/v37.2 (1 Agu, Claude — Eddy sudah kembangkan sendiri s/d v37) — Sisa tagihan klien di kartu project, CLIENT-ONLY, sw.js v78→v79:** ⚠️ File di E:\Mirror sudah maju ke v37 (Eddy push v35 pengaman kasbon/TIPE_LOG, v36 "Lunas"→"Abaikan", v37 import excel bersen fix) — edit Claude masuk di atas v37. (a) v37.1: baris "Sisa belum dibayar klien" (rupiah, `eff − cf.masuk`) di projCard, kuning/hijau. (b) **v37.2 fix (lapor Eddy):** cf.masuk hitung SEMUA pemasukan → refund barang (Wishnu) keliru dihitung sbg bayar klien. Helper baru `projMasukKlien(nama)` = getProjCF.masuk − Σ(pemasukan project ber-kelompok ≠ PEMASUKAN, dari state.txns). ⚠️ Batas: refund di luar 120 hari (tak termuat state.txns) tak ikut terpotong — untuk kasus lama pindahkan kategori refund ke kelompok non-PEMASUKAN + refund biasanya recent jadi aman. **ACTION Eddy: set kategori refund Wishnu ke kelompok PROJECT (bukan PEMASUKAN).** APP_VERSION→v37.2, sw.js → v79. ⚠️ Catatan: fitur Bunga Harian v34 pakai kategori "Bunga Bank"/"Pajak Bunga" tapi Eddy v35 seragamkan ke "Bunga"/"Pajak" — belum disesuaikan.
- **⚠️⚠️ INSIDEN DOBEL PEMBAYARAN CC (22 Jul) — KESALAHAN CLAUDE, WAJIB BACA:** Eddy lapor ada 2 txn 13.271.519 di CC-MAYBANK-BMW, satu ber-tag "REKONSILIASI". **Akar: pengecekan 8 Jul KELIRU** — waktu itu Claude memfilter pembayaran CC dengan `'Bayar CC' in KATEGORI`, padahal transfer bank→kartu berkategori **'Transfer Keluar'/'Transfer Masuk'** dan teks "Bayar CC" hanya ada di **NOTES** → disimpulkan "pembayaran hilang" padahal Eddy sudah mencatatnya sendiri (7-8 Jul, tercatat rapi + TRANSFER_LOG) → `rapikanJul8` mencatat ulang → **4 dari 5 dobel** (BMW 13.271.519, INFINITE 1.478.659, CIMB-JCB 2.272.675, ACCOR 2.720.980 [catatan Eddy utk ACCOR malah 3.987.675 — lebih besar, punya Eddy yang benar]). Hanya **BNI-JCB 4.046.657 yang TIDAK dobel** (Eddy cuma catat 20.000). Dampak: saldo SEABANK-RONAH & CIMB understated total **21.069.833** (ini juga sebab CIMB sempat minus 224.632 — bukan setoran belum tercatat spt dugaan awal). **Deliverable: `hapus-dobel-22jul2026.gs`** (fungsi `hapusDobelJul22`: backupNow → hapus 8 baris ber-TAG kecuali yang ber-NOTES 'CC-BNI-JCB' → hapus TRANSFER_LOG pasangan by REF TRK8J…). **ATURAN BARU: cek pembayaran CC WAJIB lewat NOTES (`'Bayar CC' in NOTES`) atau sheet TRANSFER_LOG — JANGAN pakai kolom KATEGORI.**
- **Klarifikasi UX (22 Jul):** (a) tombol **"Bayar"** (hijau) = satu-satunya yang mencatat uang keluar; **"Lunas"** (abu) hanya menghapus pengingat, TIDAK mencatat apa pun (inilah sebab insiden 8 Jul). (b) Tagihan CC baru (BMW 51.884.516, tempo 7 Agu, dikunci 22 Jul 13:41) TIDAK muncul di banner atas dashboard — **by design**: `renderCCBillReminder` hanya menampilkan tagihan ≤7 hari; tagihannya tetap tampil di strip kartu section "Kartu Kredit" & naik ke banner otomatis pada 31 Jul. (c) ⚠️ PR terbuka: rekonsiliasi BMW (marks ~70jt vs tagihan; bill baru 51,8jt) menunggu statement cetakan Maybank.
- **v34 (22 Jul) — Input massal Bunga Harian, CLIENT-ONLY, sw.js v73:** permintaan Eddy (Seabank bunga+pajak tiap hari, males klik satu-satu). Menu **Lainnya → Bunga Harian** (`openBungaHarian`, drawer): pilih rekening (default auto SEABANK-EDDY), tanggal baris pertama (default tgl 1 bulan berjalan), textarea 1 baris = 1 hari (`bunga` atau `bunga pajak`; parser id-ID titik=ribuan koma=desimal), checkbox **pajak otomatis 20%** (override per baris via kolom 2), preview live (n hari, rentang tanggal, total bunga/pajak/netto), confirm → **`addTxnBatch`** sekali kirim (2 txn/hari: Pemasukan "Bunga Bank" + Pengeluaran "Pajak Bunga", kelompok FINANCIAL self-provision via newKats) → syncAll. Verifikasi: parser & tanggal lintas bulan PASS, blok vm.Script OK, addTxnBatch server generate ID 'IMP…'+TIPE_LOG=JENIS ✓. APP_VERSION→v34, sw.js → v73.
- **v33.6 (22 Jul) — Fix warna header invoice hilang saat Simpan PDF, CLIENT-ONLY, sw.js v72:** browser membuang background saat print kecuali dipaksa. Fix: `-webkit-print-color-adjust:exact;print-color-adjust:exact` di rule `*` invHTML. (Sebelumnya kadang tampak hijau hanya bila user mencentang "Background graphics" di dialog print.) APP_VERSION→v33.6, sw.js → v72.
- **v33.5 (13 Jul) — Chart Cashflow Project auto-rotasi, CLIENT-ONLY, sw.js v71:** keluhan Eddy: chart tampil project itu-itu saja. Akar: `projWithTxn = filter(hasProjActivity).slice(0,6)` = 6 pertama urutan MASTER_PROJECT (terlama). Fix: sort STATUS 'Jalan' dulu → lalu `_lastAct` (tanggal txn terbaru per project dari state.txns) desc → slice 6. Judul +"· 6 paling aktif". Auto-update tiap sync/txn baru. Catatan konsultasi: refund jaminan sewa → kategori "Refund" pindah kelompok PROJECT (keputusan Eddy; filter invoice/termin hanya tangkap kelompok PEMASUKAN → bersih retroaktif; preseden kategori lama "Refund - Uang Jaminan" memang PROJECT). APP_VERSION→v33.5, sw.js → v71.
- **v33.4 (13 Jul) — "Telah diterima" ikut DP tanpa termin + format tanggal, CLIENT-ONLY, sw.js v70:** kasus Wishnu: DP dicatat langsung sbg TXN (tanpa termin) → tak muncul di invoice. genInvoice jadi **async**: telah = termin Lunas (excl yang ditagih) + `getTxns project` → Pemasukan kelompok PEMASUKAN **tanpa tag [PIUTANG]** (anti-dobel dgn txn hasil Tandai Lunas), label dari KATEGORI. Baris se-tanggal & se-label di-merge (DP Tri Kerta + koreksi 125rb → tampil satu 5jt utuh). Format baris: **"Nama — diterima <tglIndo>"**. Toast "Menyiapkan invoice…" saat fetch. vm.Script OK. APP_VERSION→v33.4, sw.js → v70.
- **v33.3 (13 Jul) — Invoice: seksi "Telah diterima" + spacing tabel, CLIENT-ONLY, sw.js v69:** keluhan Eddy: rincian pembayaran sebelumnya terjepit di dalam sel barang (font 11px, tampak terpotong) + isi tabel mepet. Fix invHTML: (a) `telahBlock` dipindah jadi seksi sendiri SETELAH TERBILANG — heading "TELAH DITERIMA SEBELUMNYA:", tabel `.paidtbl` (12px, lebar 62%, garis bawah tipis, nominal rata kanan bold) + baris **Total telah diterima** (garis atas hijau); `break-inside:avoid` biar tak terbelah halaman; (b) padding tabel item: th 7px→10px, td 8px→12px 10px + line-height 1.55. APP_VERSION→v33.3, sw.js → v69.
